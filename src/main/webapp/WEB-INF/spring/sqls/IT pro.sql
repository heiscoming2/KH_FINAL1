...
CREATE TABLE M_RESUME (
    ...
    R_IMG_NO NUMBER NOT NULL,  --이미지 테이블no
...
CREATE TABLE EDUCATION (
    ...
    ED_GRADUATION VARCHAR2(50) NOT NULL, --1-졸업 / 2-휴학 / 3-중퇴 / 4-졸업예정
    ...
    CONSTRAINT ED_GRADUATION_VAL CHECK(ED_GRADUATION IN('졸업','휴학','중퇴','졸업예정')),
...
CREATE TABLE AD_REQUEST (
   AD_NO NUMBER,
   AD_COMTYPE VARCHAR2(15),
   AD_COMNAME VARCHAR2(15),
   AD_STARTDATE DATE,
   AD_ENDDATE DATE,
   AD_URL VARCHAR2(50),
   AD_PRICE NUMBER,
   ...
   AD_IMG_PATH VARCHAR2(300),
    M_NO NUMBER,
   BD_NO NUMBER,
    CONSTRAINT AD_NO_FK PRIMARY KEY(AD_NO)
);
...
CREATE TABLE KAKAO_PAY (
   ...
   PRICE NUMBER,
    CONSTRAINT KAKAO_PAY_AD_SEQ_PK PRIMARY KEY(PAY_SEQ)
);
...
SELECT * FROM BOARD;
		SELECT M.M_IMG_PATH, M.M_IMG, M.M_NICKNAME, B.BD_CREATEDDATE, B.BD_MODIFYDATE, 
		       ...
		       (SELECT COUNT(BD_NO) FROM BOARDRECOMMAND R WHERE R.BD_NO=11) BD_RECOMMANDCOUNT, 
		       A.M_NO, A.AD_COMTYPE, A.AD_COMNAME, A.AD_STARTDATE, A.AD_ENDDATE, AD_PRICE, AD_URL 
		FROM BOARD B, IT_MEMBER M, AD_REQUEST A
		WHERE B.M_NO = M.M_NO
		  AND B.M_NO = A.M_NO
		  AND B.BD_NO = 11;
...
SELECT M.M_IMG_PATH, M.M_IMG, M.M_NICKNAME, B.BD_CREATEDDATE, B.BD_MODIFYDATE, 
		        B.BD_WRITERIP, B.BD_NO, B.BD_TITLE, B.BD_CONTENT, 
		       (SELECT COUNT(BD_NO) FROM BOARDRECOMMAND R WHERE R.BD_NO=0) BD_RECOMMANDCOUNT, 
		       A.M_NO, A.AD_COMTYPE, A.AD_COMNAME, A.AD_STARTDATE, A.AD_ENDDATE, AD_PRICE, AD_URL 
		FROM BOARD B, IT_MEMBER M, AD_REQUEST A
		WHERE B.M_NO = M.M_NO
		  AND B.M_NO = A.M_NO
...
CREATE TABLE STUDY (
   ...
   CONSTRAINT STUDY_BDNO_PK PRIMARY KEY(BD_NO)   
		...
		SELECT M.M_IMG_PATH, M.M_IMG, M.M_NICKNAME, B.BD_CREATEDDATE, B.BD_MODIFYDATE, 
		        B.BD_WRITERIP, B.BD_NO, B.BD_TITLE, B.BD_CONTENT, 
		       (SELECT COUNT(BD_NO) FROM BOARDRECOMMAND R WHERE R.BD_NO=16) BD_RECOMMANDCOUNT, 
		       A.M_NO, A.AD_COMTYPE, A.AD_COMNAME, A.AD_STARTDATE, A.AD_ENDDATE, A.AD_PRICE, A.AD_URL,
		       A.AD_IMG, A.AD_IMG_PATH
		FROM BOARD B, IT_MEMBER M, AD_REQUEST A
		WHERE B.M_NO = M.M_NO
		  AND B.BD_NO = A.BD_NO
		  AND B.BD_NO = 16;
...
--학력사항
    ...
    VALUES(1004,EDUCATION_SEQ.NEXTVAL,to_date('20210305'),NULL,'서울대학교 대학원','졸업예정','컴퓨터공학과','대학원');
      INSERT INTO EDUCATION
